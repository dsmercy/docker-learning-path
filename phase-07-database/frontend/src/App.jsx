...
export default function App() {
  ...
  const [editing,  setEditing]  = useState(null)   // MongoDB _id being edited
  ...
  return (
    <div style={s.page}>
      <header style={s.header}>
        <h1>Phase 7 — Product Management V2</h1>
        <p>Three-service stack: <code>frontend</code> · <code>api</code> · <code>mongodb</code></p>
      ...
      <section>
        ...
        {products.length > 0 && (
          <table style={s.table}>
            ...
            <tbody>
              {products.map(p => (
                <tr key={p._id} style={editing === p._id ? s.rowHighlight : undefined}>
                  ...
                  <td style={s.td}>
                    ...
                    <button style={s.btnDelete} onClick={() => deleteProduct(p._id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <p style={s.note}>
          ✅ Data is persisted in a named Docker volume. Run <code>docker compose down</code> +{' '}
          <code>docker compose up</code> — products survive.
        </p>
      </section>
    </div>
  )
...
const s = {
  ...
  header:       { marginBottom: 24, borderBottom: '2px solid #10b981', paddingBottom: 12 },
  ...
  btnPrimary:   { background: '#10b981', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 16px', cursor: 'pointer', fontSize: 14 },
  ...
  rowHighlight: { background: '#ecfdf5' },
  badge:        { background: '#d1fae5', color: '#065f46', borderRadius: 4, padding: '2px 7px', fontSize: 12 },
