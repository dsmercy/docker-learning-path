...
export default function App() {
  ...
  const submit = async (e) => {
    ...
    const res = await fetch(editing ? `${API_URL}/products/${editing}` : `${API_URL}/products`, {
  ...
  return (
    <div style={s.page}>
      <header style={s.header}>
        <h1>Phase 12 — CI/CD Pipeline</h1>
        <p>This image was built, scanned by Trivy, and pushed by GitHub Actions.</p>
        <p><small>API: {API_URL} | Build: {import.meta.env.VITE_BUILD_SHA || 'local'}</small></p>
      </header>
              ...
              {products.map(p => (
                <tr key={p._id}>
                  ...
                  <td style={s.td}>
                    ...
                    <button style={s.btnDel} onClick={async () => { if(window.confirm('Delete?')) { await fetch(`${API_URL}/products/${p._id}`, {method:'DELETE'}); load() } }}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
const s = {
  ...
  header: { marginBottom: 24, borderBottom: '2px solid #16a34a', paddingBottom: 12 },
  ...
  btn: { background: '#16a34a', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 16px', cursor: 'pointer', fontSize: 14 },
